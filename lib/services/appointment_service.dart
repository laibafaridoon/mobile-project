...
class AppointmentService {
  ...
  static Future<Appointment> bookAppointment({
    ...
    try {
      ...
      final docPrefix = doctor.name
          .split(' ')
          .last
          .substring(0, doctor.name.split(' ').last.length >= 3 ? 3 : 1)
          .toUpperCase();
      ...
      await NotificationService.addNotification(
        ...
        body: 'Appointment with ${doctor.name} on ${_formatDate(date)} at $timeSlot',
        type: 'appointment',
        userId: patientId,
      );
  ...
  static Future<List<Appointment>> _getAppointmentsForDoctor(String doctorId, DateTime date) async {
    try {
      final query = await FirebaseService.queryCollection(
        collection: 'appointments',
        field: 'doctorId',
  ...
  static Future<Appointment?> rescheduleAppointment(String appointmentId, DateTime newDate, String newSlot) async {
    try {
      final aptDoc = await FirebaseService.getDocument(collection: 'appointments', docId: appointmentId);
      if (!aptDoc.exists) return null;

      final appointment = Appointment.fromMap(aptDoc.data()! as Map<String, dynamic>, appointmentId);
      final updated = appointment.copyWith(date: newDate, timeSlot: newSlot, status: 'Pending');

      await FirebaseService.updateDocument(collection: 'appointments', docId: appointmentId, data: updated.toMap());
  ...
  static Future<bool> cancelAppointment(String appointmentId) async {
    try {
      ...
      // --- REFUND CALCULATION (10% DEDUCTION) ---
      double refundAmount = 0;
      if (appointment.amountPaid != null && appointment.amountPaid! > 0) {
        // 10% cut, 90% return
        refundAmount = appointment.amountPaid! * 0.90;
      }
      ...
      // Create notification with refund details
      String refundMessage = refundAmount > 0
          ? ' Your refund of PKR ${refundAmount.toStringAsFixed(2)} has been processed (after 10% deduction).'
          : '';

      await NotificationService.addNotification(
        title: 'Appointment Cancelled',
        body: 'Your appointment with ${appointment.doctorName} was cancelled.$refundMessage',
        type: 'appointment',
        userId: appointment.patientId,
      );
  ...
  static Future<bool> updateStatus(String appointmentId, String newStatus) async {
    try {
      final aptDoc = await FirebaseService.getDocument(collection: 'appointments', docId: appointmentId);
      if (!aptDoc.exists) return false;
      ...
      await FirebaseService.updateDocument(collection: 'appointments', docId: appointmentId, data: {'status': newStatus});
      ...
      if (newStatus == 'Confirmed') {
        ...
        body = 'Your appointment with ${appointment.doctorName} is confirmed for ${_formatDate(appointment.date)} at ${appointment.timeSlot}';
      } else if (newStatus == 'Your Turn Next') {
        title = 'You\'re Next!';
        body = 'You are next for ${appointment.doctorName} at ${appointment.roomNumber}';
      } else if (newStatus == 'In Progress') {
        title = 'Consultation Started';
        body = 'Dr. ${appointment.doctorName} is ready for you at ${appointment.roomNumber}';
      ...
      if (title != null && body != null) {
        await NotificationService.addNotification(title: title, body: body, type: 'queue', userId: appointment.patientId);
      }
  ...
  Future<Appointment?> getAppointmentById(String appointmentId) async {
    try {
      final doc = await FirebaseService.getDocument(collection: 'appointments', docId: appointmentId);
  ...
  static Future<bool> acceptAppointment(String appointmentId) async {
    try {
      ...
      final existingAppointments = await _getAppointmentsForDoctor(appointment.doctorId, appointment.date);
      final position = existingAppointments.length + 1;
      final waitTime = position * 15;

      final lastWord = appointment.doctorName.split(' ').last;
      final docPrefix = lastWord.substring(0, lastWord.length >= 3 ? 3 : lastWord.length).toUpperCase();
      final token = '$docPrefix-${100 + position}';
      final room = _assignRoom(appointment.doctorId);
  ...
  static Future<bool> rejectAppointment(String appointmentId) async {
    try {
      ...
      await FirebaseService.updateDocument(collection: 'appointments', docId: appointmentId, data: {'status': 'Cancelled'});

      await NotificationService.addNotification(
        title: 'Appointment Cancelled',
        body: 'Dr. ${appointment.doctorName} was unable to accept your request.',
        type: 'appointment',
        userId: appointment.patientId,
      );
